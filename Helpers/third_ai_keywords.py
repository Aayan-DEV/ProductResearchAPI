...
"""
SEO Keyword Generator using Google API (Gemini)
- Uses Google Generative AI API (no local models)
- Fast API-style function: generate_keywords_for_title_api(title)
- Batch processing support for demand summaries
"""
...
def ensure_llm_loaded(project_root: Optional[Path] = None):
    """Load Google API configuration. No local models."""
    api_key = (os.getenv("GOOGLE_API_KEY") or "").strip()
    if not api_key:
        raise RuntimeError("GOOGLE_API_KEY is missing. Set GOOGLE_API_KEY environment variable.")
...
def ensure_llm_loaded_singleton(project_root: Optional[Path] = None):
    """Singleton pattern for Google API client."""
...
def generate_keywords_for_title_api(title: str) -> List[str]:
    ...
    Uses Google API - fast and no local models required.
    """
